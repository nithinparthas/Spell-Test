...
bimage, gimage, grimage, rimage, textbgnd_img, eye  = create_tile(charlist) # Create the basic flashboard
...
img = cv2_get_full_image(bimage, gimage, rimage, grimage, textbgnd_img, textsrcimg, 12, 0, [0,0], 0)
...
while count < MAXCOUNT:
   ...
   while max(dist) < DIST_THRESHOLD:
      ...
      img = cv2_get_full_image(bimage, gimage, rimage, grimage, textimg, textsrcimg, rowcol_idx, flag_fix_distraction, distraction_list, bspace_char_flag)
      cv2.imshow(winname,img)
...
#    print("Idx=%d Prob is %f" %(idx,prob))
   ...
   textimg = display_text(declist, textbgnd_img, 1) 
   img = cv2_get_full_image(bimage, gimage, rimage, grimage, textimg, textsrcimg, 12, flag_fix_distraction, distraction_list, 0)
cv2.imshow(winname,img)
...
print("Total Flashboard Simulation Time %1.2f sec, Avg Time/Char = %1.2f sec, err=%d, err_rate=%1.2f" %((simendtime - simstarttime),(simendtime - simstarttime)/MAXCOUNT, finalerr, finalerr/len(text)))
