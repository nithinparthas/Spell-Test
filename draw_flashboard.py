...
def cv2_get_full_image(bim, gim, rim, grim, textimg, textsrcimg, idx, flag_fix_distraction, distraction_list, bspace_char_flag):
    ...
    if not flag_fix_distraction: # Colorize the attended row/col based on feedback and flag
        bkim = bim
    elif (distraction_list[-2] and distraction_list[-1]) or (bspace_char_flag): # Change background colors based on last two distraction indication
        bkim = grim
    else:
        bkim = bim
    ...
    board = cv2.vconcat([textsrcimg, board, textimg])
    return(board)
 ...
 #   crop_image =gim1()
  #  cv2.imshow('debug14', crop_image)
    return([bim0, bim1, bim2, bim3, bim4, bim5, bim6, bim7, bim8, bim9, bim10, bim11],
            ...
            [rim0, rim1, rim2, rim3, rim4, rim5, rim6, rim7, rim8, rim9, rim10, rim11], textbgnd_img, eye)
